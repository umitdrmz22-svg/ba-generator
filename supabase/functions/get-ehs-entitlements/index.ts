...
const MODULE_PRODUCTS=['ehs_ba_monthly','ehs_fluchtplan_monthly','ehs_brandschutzordnung_monthly','ehs_gefahrstoffkataster_monthly','ehs_dokumentmanagement_monthly','ehs_unfallmanagement_monthly'] as const;
...
export default{async fetch(req:Request){
  ...
  const authHeader=req.headers.get('Authorization'); if(!authHeader?.startsWith('Bearer '))return response({error:'UNAUTHORIZED'},401);
  const supabaseUrl=Deno.env.get('SUPABASE_URL'); const publishableKeys=JSON.parse(Deno.env.get('SUPABASE_PUBLISHABLE_KEYS')??'{}'); const secretKeys=JSON.parse(Deno.env.get('SUPABASE_SECRET_KEYS')??'{}');
  const publishableKey=publishableKeys.default??Deno.env.get('SUPABASE_ANON_KEY'); const secretKey=secretKeys.default??Deno.env.get('SUPABASE_SERVICE_ROLE_KEY');
  ...
  const{data:userData,error:userError}=await userClient.auth.getUser(); const user=userData.user; if(userError||!user)return response({error:'UNAUTHORIZED'},401);
  const admin=createClient(supabaseUrl,secretKey,{auth:{persistSession:false,autoRefreshToken:false}});
  const access=new Map<string,ModuleAccess>(); for(const productId of MODULE_PRODUCTS)access.set(productId,{productId,active:false,mode:'none',sources:[],works:[],expiresAt:null});

  const{data:binding,error:bindingError}=await admin.from('ehs_personal_werk_bindings').select('organization_id,werk_id,locked_at').eq('user_id',user.id).maybeSingle();
  if(bindingError)return response({error:'ENTITLEMENT_QUERY_FAILED'},500);
  let personalWork:WorkAccess|null=null;
  if(binding?.werk_id){
    const{data:werk,error:werkError}=await admin.from('ehs_werks').select('id,organization_id,name,code').eq('id',binding.werk_id).maybeSingle();
    if(werkError)return response({error:'ENTITLEMENT_QUERY_FAILED'},500);
    if(werk&&String(werk.organization_id)===String(binding.organization_id)) personalWork={id:String(werk.id),organizationId:String(werk.organization_id),name:String(werk.name??'Werk'),code:String(werk.code??''),role:'owner',seatType:'editor'};
  }
  ...
  for(const row of personalRows??[]){
    ...
    const item=access.get(productId); if(!item)continue; activePersonalProducts.push(productId);
    if(personalWork){item.active=true;item.mode='edit';if(!item.sources.includes('google_play'))item.sources.push('google_play');item.expiresAt=row.expires_at??null;pushWork(item,personalWork);}
  }
  if(legacyAllAccess&&personalWork)for(const item of access.values()){item.active=true;item.mode='edit';if(!item.sources.includes('legacy_all_access'))item.sources.push('legacy_all_access');pushWork(item,personalWork);}
  ...
  for(const membership of memberships??[]){
    const werk=werkById.get(String(membership.werk_id)); if(!werk)continue;
    if(!validUntil(String(werk.billing_status??''),(werk.current_period_end as string|null|undefined)??null))continue;
    const seatType=String(membership.seat_type??'reader'); const mode:AccessMode=seatType==='editor'?'edit':'read'; const modules=Array.isArray(werk.licensed_modules)?werk.licensed_modules.map(String):[];
    for(const productId of modules){const item=access.get(productId);if(!item)continue;item.active=true;item.mode=maxMode(item.mode,mode);if(!item.sources.includes('corporate_werk'))item.sources.push('corporate_werk');pushWork(item,{id:String(werk.id),organizationId:String(werk.organization_id??''),name:String(werk.name??''),code:String(werk.code??''),role:String(membership.role??''),seatType});}
  }
  const modules=[...access.values()]; const allWorks=Array.from(new Map(modules.flatMap(m=>m.works).map(w=>[w.id,w])).values());
  return response({ok:true,userId:user.id,modules,personal:{activeProducts:activePersonalProducts,legacyAllAccess,bindingRequired:(activePersonalProducts.length>0||legacyAllAccess)&&!personalWork,binding:personalWork?{organizationId:personalWork.organizationId,werkId:personalWork.id,name:personalWork.name,code:personalWork.code,lockedAt:binding?.locked_at??null}:null},corporate:{works:allWorks.filter(w=>!personalWork||w.id!==personalWork.id)}});
}};
